...
- 更新提示增加 GitHub 与 npm 双边版本校核：只有该版本在 GitHub master 与 npm 镜像**同时存在**时才提示更新。
- GitHub 领先于 npm 时，自动改为提示 npm 已发布且两边均存在的最新版本，并在诊断日志记录差异（如"GitHub 已到 0.1.3-alpha.1，npm 尚未发布"）。
- 显式指定版本下载（安装器选择）前先校核 npm 是否已发布该版本，未发布时直接给出明确错误，不再发起必然失败的下载。
